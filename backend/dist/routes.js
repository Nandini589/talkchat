...
const messageSelect = "*, users!messages_sender_id_fkey(id, name, avatar_url), message_reads(user_id, read_at, users!message_reads_user_id_fkey(id, name, avatar_url))";
...
export const ensureGeneralMembership = async (userId) => {
    const { data: general, error: conversationError } = await supabase
        .from("conversations")
        .select("id")
        .eq("name", "General")
        .maybeSingle();
    if (conversationError || !general)
        return;
...
const markConversationMessagesRead = async (conversationId, userId) => {
